...
- **`/split`**: Используется, когда план надёжен. Планировщик разбивает задачу на подзадачи с полными prompt packet и отправляет их на исполнение.
...
1. **Read-first:** read-heavy подзадачи (исследование, воспроизведение, сбор контекста) запускаются первыми.
2. **Write после read:** write-heavy подзадачи запускаются только после завершения связанных read-задач.
3. **Последовательность при пересечении:** подзадачи, затрагивающие один файл, выполняются строго последовательно.
4. **Параллельность при раздельности:** подзадачи с непересекающимися файлами и модулями могут выполняться параллельно.
5. **Граф зависимостей** описывается в plan-файле; dispatch следует этому графу.

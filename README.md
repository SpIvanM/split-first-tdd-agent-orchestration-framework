...
graph TD
    ...
    subgraph "Цикл исполнения (Главный агент)"
        ...
        subgraph "Внутренний цикл субагента (TDD + Регрессия)"
            L --> M["Локальный падающий тест"]
            M --> N["Доработка кода"]
            N --> O["Локальный зеленый тест"]
            O --> P["Слияние в регресс-пакет"]
            P --> Q["Запуск единого регресс-теста (Green)"]
            Q -- "Retry" --> N
        end
        
        Q -- "Success" --> R{"Последняя задача?"}
        R -- "Нет" --> K
        
        L -- "Критическая ошибка" --> T["Перепланирование"]
        T --> F
    end
    
    R -- "Да" --> U["Финальная верификация (Suite Green)"]
    U --> V["Merge / Завершено"]
```
...
4. **Внутренний TDD и Регрессия**: Субагент внутри своего прохода запускает падающий тест, правит код и добивается прохождения локального теста. После этого он вливает свои тесты в единый регрессионный пакет и добивается прохождения **всего пакета** (Green) перед возвратом результата.
5. **Нарастающий регресс**: Каждый успешный цикл субагента завершается подтверждением того, что проект в целом остается работоспособным.
